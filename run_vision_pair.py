#!/usr/bin/env python3
"""
VisionIAS Full Exam Processor (Questions + Solutions)
...
def main():
    parser = argparse.ArgumentParser(
        description="VisionIAS Full Exam Processor (Questions + Solutions)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_vision_pair.py --q_pdf questions.pdf --s_pdf solutions.pdf
  python run_vision_pair.py --q_pdf q.pdf --s_pdf s.pdf --output_dir ./results
        """
    )
    ...
    parser.add_argument('--output_dir', default="./output/merged_exams", 
                        help="Directory for final output (default: ./output/merged_exams)")
    
    args = parser.parse_args()
    setup_logger()
    logger = logging.getLogger("VisionPairRunner")
    ...
    # Use OutputManager to create a session folder
    # We'll use the question PDF name for the session
    out_man = OutputManager(args.output_dir, args.q_pdf, {"MODE": "MERGED_PAIR"})
